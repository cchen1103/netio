...
def main():
    ...
    with sniff_sock() as s:
        ...
        for i in range(1000):
            ...
            nc.update(decoder_eth(data)[:-1])
            nc.update(decoder_ip(data)[:-1])
            nc.update(decoder_tcp(data)[:-1])
            nc.update(decoder_udp(data)[:-1])
            tnc.update(decoder_eth(data)[:-1])
            tnc.update(decoder_ip(data)[:-1])
            tnc.update(decoder_tcp(data)[:-1])
            tnc.update(decoder_udp(data)[:-1])
            tc.update(decoder_tcp(data)[:-1])
            ttc.update(decoder_tcp(data)[:-1])
