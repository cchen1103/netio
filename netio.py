...
def main():
    ...
    with sniff_sock() as s:
        ...
        for i in range(1000):
            ...
            nc.update(decoder.decode_eth(data)[:-1])
            nc.update(decoder.decode_ip(data)[:-1])
            nc.update(decoder.decode_tcp(data)[:-1])
            nc.update(decoder.decode_udp(data)[:-1])
            tnc.update(decoder.decode_eth(data)[:-1])
            tnc.update(decoder.decode_ip(data)[:-1])
            tnc.update(decoder.decode_tcp(data)[:-1])
            tnc.update(decoder.decode_udp(data)[:-1])
            tc.update(decoder.decode_tcp(data)[:-1])
            ttc.update(decoder.decode_tcp(data)[:-1])
